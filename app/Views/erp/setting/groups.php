<div class="alldiv flex widget_title">
    ...
    <div class="title_right">
        <a href="#" id="groups_modal_invoker1" class="btn bg-success modalBtn"><i class="fa fa-plus"></i> Group </a>
    </div>
</div>

<div class="alldiv">
    <div class="datatable" data-ajax-url="<?php echo url_to('erp.setting.ajaxgroupsresponse') . '?'; ?>">
        <!-- <div class="filterBox">
            <div class="flex">
...
<!--MODALS-->
<div class="modal" id="groups_addedit_modal" role="dialog">
    <div class="modalbody">
        <h2 class="modalTitle">Groups</h2>
        <?php
        echo form_open(url_to('erp.setting.groupaddedit'), array(
            "id" => "groups_addedit_form",
            "class" => "flex"
        ));
        ?>
        <input type="hidden" name="group_id" id="f_group_id" value="0" />
            ...
            <div class="form-group field-required ">
                ...
                <div class="selectBox poR">
                    <div class="selectBoxBtn flex">
                        ...
                        <button class="close" type="button"><i class="fa fa-close"></i></button>
                        <button class="drops" type="button"><i class="fa fa-caret-down"></i></button>
                        <input type="hidden" id="f_group_related" class="selectBox_Value field-check" name="related_to" value="">
                    </div>
                    <ul role="listbox" class="selectBox_Container alldiv">
                        <?php
                        foreach ($related_to as $key => $value) {
                        ?>
                            <li role="option" data-value="<?php echo $key; ?>"><?php echo ucfirst($value); ?></li>
                        <?php
                        }
                        ?>
                    </ul>
        ...
        <div class="form-width-1 ">
            <div class="form-group textRight ">
                ...
                <button class="btn bg-primary" type="button" id="groups_addedit_btn">Save</button>
            </div>
        </div>
        </form>
...
<script type="text/javascript">
    ...
    let selectbox = new SelectBox(document.querySelector("#groups_addedit_modal .selectBox"));
    selectbox.init();
    closer.register_shutdown(selectbox.shutdown, selectbox.get_container());

    let modal_box = new ModalBox(document.getElementById("groups_addedit_modal"));
    modal_box.init();

    document.getElementById("groups_modal_invoker1").onclick = (evt) => {
                ...
                if (xhr !== null && xhr !== undefined) {
                    ...
                    xhr.onreadystatechange = (evt) => {
                        if (xhr.readyState == 4 && xhr.status == 200) {
                            ...
                            if (json['error'] == 0) {
                                let data = json['data'];
                                document.getElementById("f_group_id").value = data['group_id'];
                                document.getElementById("f_group_name").value = data['group_name'];
                                document.getElementById("f_group_related").value = data['related_to'];
                                selectbox.construct();
                                modal_box.show_modal();
                            }
                        }
    ...
    let form = document.getElementById("groups_addedit_form");
    let validator = new FormValidate(form);

    let lock = false;
    document.getElementById("groups_addedit_btn").onclick = function(evt) {
        if (!lock) {
            lock = true;
            validator.validate(
                (params) => {
