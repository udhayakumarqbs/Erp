...
<div class="alldiv import-section">
    ...
    <div class="textRight download-template">
        <a href="<?php echo url_to('erp.inventory.finishedgoodimporttemplate'); ?>" target="_BLANK" class="btn bg-success">Download Template</a>
    </div>
    ...
    <?php
    echo form_open_multipart(url_to('erp.inventory.finishedgoodimport'), array(
        "class" => "flex",
        "id" => "finishedgood_import_form"
    ));
    ?>
    ...
    <div class="form-width-1">
        <div class="form-group textRight">
            ...
            <button class="btn bg-primary" type="button" id="finishedgood_import_submit">Import</button>
        </div>
    </div>
    <?php
    echo form_close();
...
<script type="text/javascript">
    ...
    let form = document.getElementById("finishedgood_import_form");
    let validator = new FormValidate(form);

    let lock = false;
    document.getElementById("finishedgood_import_submit").onclick = function(evt) {
        if (!lock) {
            lock = true;
            validator.validate(
                (params) => {
