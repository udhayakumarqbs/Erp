...
<div class="alldiv">
    <?php
    echo form_open(url_to('erp.sale.expense.invoice.add', $expense_existing_data->id), array(
        "id" => "invoice_add_form",
        "class" => "flex"
    ));
    ?>
    <div class="form-width-2">
        <div class="form-group field-ajax " data-ajax-url="<?= url_to('sale.invoice.add.fetchcode') . '?'; ?>">
            <?php
            $currentYear = date('Y');
            $newCode = "INV-$currentYear" . sprintf('%03d', $maxCode + 1);
            ?>
    ...
    <div class="form-width-2">
        <div class="form-group field-required">
            ...
            <input type="date" name="invoice_date" id="invoice_date" value="<?php echo date("Y-m-d"); ?>" class="form_control field-check" />
            <p class="error-text"></p>
        </div>
    </div>
    <div class="form-width-2">
        <div class="form-group field-required">
            <?php $customer = isset($expense_existing_data->customer_id) ? $expense_existing_data->customer : "select customer" ?>
            ...
            <div class="ajaxselectBox poR" id="customer_ajax_select_box" data-ajax-url="<?= url_to('erp.sale.getCustomer'); ?>">
                <div class="ajaxselectBoxBtn flex">
                    <div class="textFlow" data-default="<?= $customer ?>"><?= $customer ?></div>
                    <button class="close" type="button"><i class="fa fa-close"></i></button>
                    <button class="drops" type="button"><i class="fa fa-caret-down"></i></button>
                    <input type="hidden" class="ajaxselectBox_Value field-check" name="cust_id" id="cust_id" value="<?= $expense_existing_data->customer_id  ?>">
                </div>
    ...
    <div class="form-width-2">
        <div class="form-group field-money">
            ...
            <input type="text" id="invoice_trans_charge" name="trans_charge" value="0" class="form_control field-check" />
            <p class="error-text"></p>
        </div>
    </div>
    <div class="form-width-2">
        <div class="form-group field-required">
            ...
            ?>
            <input type="date" name="invoice_expiry" id="seventh_day" class="form_control field-check" value="<?= $sevenDaysLater ?>" />
            <p class="error-text"></p>
        </div>
    </div>
    <div class="form-width-2">
    ...
    <div class="form-width-2">
        <div class="form-group field-required">
            ...
            <div class="selectBox poR" id="invoice_status">
                <div class="selectBoxBtn flex">
    ...
    <div class="form-width-1">
        <table class="table">
            ...
            <tbody id="invoice_items_holder">
                <?php $tax_1 = isset($expense_existing_data->tax_1_name) ? (($expense_existing_data->tax_1_per / 100) * $expense_existing_data->amount)  : 0 ?>
                <?php $tax_2 = isset($expense_existing_data->tax_2_name) ? (($expense_existing_data->tax_2_per / 100) * $expense_existing_data->amount)  : 0 ?>
                <?php $amount = $expense_existing_data->amount ?>
                <input type="hidden" name="expense_amount" value="<?= $amount ?>">
                <input type="hidden" name="related_id" value="<?= $expense_existing_data->id ?>">
                <input type="hidden" name="tax1" value="<?= $expense_existing_data->tax_1_per ?>">
                <input type="hidden" name="tax2" value="<?= $expense_existing_data->tax_2_per ?>">
                <td>1</td>
                <td><?= $expense_existing_data->cat_name ?></td>
                <td>1</td>
                <td><?= $expense_existing_data->amount ?></td>
                <?php if (isset($expense_existing_data->tax_1_name) && isset($expense_existing_data->tax_2_name)) { ?>
                    <td>
                        <?= $expense_existing_data->tax_1_per . " % " . "(" . $expense_existing_data->tax_1_name . ")"  ?><br>
                        <?= $expense_existing_data->tax_2_per . " % " . "(" . $expense_existing_data->tax_2_name . ")" ?><br>

                    </td>
                <?php } elseif (isset($existing_data->tax_1_name)) { ?>
                    <td><?= $expense_existing_data->tax_1_per . " % " . "(" . $expense_existing_data->tax_1_name . ")"  ?><br></td>
                <?php } else { ?>
                    <td><?= $expense_existing_data->tax_2_per . " % " . "(" . $expense_existing_data->tax_2_name . ")"  ?><br></td>
                <?php } ?>
                <?php $total =  isset($expense_existing_data->amount) ? $tax_1 + $tax_2 . ".00" : 0  ?>
                <td><?= $total ?></td>
                <input type="hidden" id="invoice_amount" name="total_amount" value="<?=  $total + $expense_existing_data->amount ?>" />
                <input type="hidden" id="tax" name="tax_1" value=""/>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="5"></td>
                ...
                <tr>
                    ...
                    <td id="invoice_subtotal"><?= $total + $expense_existing_data->amount ?>.00</td>
                </tr>
                <!-- <tr>
                ...
                <!-- <tr>
                    ...
                    <td><input type="text" name="discount" id="invoice_discount" value="0.00" class="form_control field-check" /></td>
                </tr> -->
                <tr>
                    <td colspan="5" class="text-right"><b>Total Inc. Tax</b></td>
                    <td id="invoice_total"><?= $total + $expense_existing_data->amount ?>.00</td>
                </tr>
            </tfoot>
        </table>
    </div>
    <div class="form-width-1">
        <div class="form-group textRight ">
            ...
            <button class="btn bg-primary" type="button" id="invoice_add_btn">Save</button>
        </div>
    </div>
    <?php
    echo form_close();
...
<script type="text/javascript">
    ...
    // Add event listener to the invoice_date input
    document.getElementById('invoice_date').addEventListener('change', function() {
        var invoiceDateValue = this.value;
        var seventhDay = new Date(invoiceDateValue);
        seventhDay.setDate(seventhDay.getDate() + 7);
        var seventhDayFormatted = seventhDay.toISOString().split('T')[0];
        document.getElementById('seventh_day').value = seventhDayFormatted;
    });
