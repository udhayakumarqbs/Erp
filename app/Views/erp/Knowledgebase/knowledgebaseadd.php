<div class="alldiv flex widget_title">
    <h3 class="">Create Knowledge Base</h3>
...
<div class="alldiv">
    ...
    <?php
    echo form_open(url_to('erp.Knowledgebaseadd'), array(
        "class" => "flex",
        "id" => "Knowledgebase_add_form"
    ));
    ?>
    <div class="form-width-3  form-control">
        <div class="form-group field-required">
            ...
            <div class="tw-flex tw-justify-between tw-items-center">
                <div>
                    <div class="checkbox checkbox-primary checkbox-inline">
                        <input type="checkbox" name="Internal_Article" id="showtostaff" <?php echo (!isset($announcement) || (isset($announcement) && $announcement->showtostaff == 1)) ? 'checked' : ''; ?>>
                        <label for="showtostaff"><?php echo 'Internal Article'; ?></label>
                    </div>
                    <div class="checkbox checkbox-primary checkbox-inline">
                        <input type="checkbox" name="Disabled" id="showtousers" <?php echo isset($announcement) && $announcement->showtousers == 1 ? 'checked' : ''; ?>>
                        <label for="showtousers"><?php echo 'Disabled'; ?></label>
                    </div>
                </div>
            </div>
            ...
            <?php echo form_textarea('articledescription',$contents,'class = "textarea form-control mb-1"'); ?>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#Knowledgebase_add_form').submit(function(event) {
    ...
    function groups_data_element(data) {
        ...
        html_element = '<option value="{id}"> {group} </option>';
        if (Object.keys(data.groups).length <= 0) {
            var html_copy = html_element;
            var empty = "--Empty--";
            html_copy = html_copy.replace('{id}',0);
            html_copy = html_copy.replace('{group}',empty);

            $("#subject_id").append(html_copy);
        }else{
