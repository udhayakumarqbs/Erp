...
<div class="alldiv">
    <?php
        echo form_open(base_url().'erp/supplier/segments-add',array(
            "class"=>"flex",
            "id"=>"segment_add_form"
        ));
    ?>
        ...
        <div class="form-width-1">
            <div class="form-group field-ajax " data-ajax-url="<?php echo base_url().'erp/supplier/segment-key-unique?' ;?>" >
                <label class="form-label">Segment Key</label>
                <input type="text" name="segment_key" class="form_control field-check" />
        ...
        <div class="form-width-1"  >
            <div class="form-group field-required ">
                ...
                <input type="text" name="segment_value[0]" class="form_control field-check" />
                <p class="error-text" ></p>
            </div>
        </div>
        ...
        <div class="form-width-1">
            <div class="form-group textRight">
                ...
                <button class="btn bg-primary" type="button" id="segment_add_submit">Save</button>
            </div>
        </div>
    </form>
</div>
...
<!--SCRIPT WORKS -->
    ...
    <script type="text/javascript">
        ...
        let segment_add_form=document.getElementById("segment_add_form");
        let prev_sibling=document.getElementById("segment_values_prev_sibling");
        let segment_value_index=1;
        let max_segment_value=5;
        let segment_value_counter=1;
        document.getElementById("segment_add_btn").onclick=(evt)=>{
            if(segment_value_counter < max_segment_value ){
                ...
                segment_add_form.insertBefore(div,prev_sibling);
                segment_value_index++;
                segment_value_counter++;
            }
        };

        segment_add_form.onclick=(evt)=>{
            let target=evt.target;
            segment_add_form.querySelectorAll(".append-icon-btn").forEach((item)=>{
        ...
        let validator=new FormValidate(segment_add_form);
        let lock=false;
        document.getElementById("segment_add_submit").onclick=function(evt){
            if(!lock){
                lock=true;
                validator.validate(
                (params)=>{
                    segment_add_form.submit();
                    lock=false;
                },
                (params)=>{
                    lock=false;
