...
<div class="alldiv">
    <?php
    echo form_open(url_to('erp.mrp.productplanningedit', $planning_id), array(
        "class" => "flex",
        "id" => "planning_edit_form"
    ));
    ?>
        ...
        <div class="form-group field-required">
            ...
            <div class="selectBox poR">
                <div class="selectBoxBtn flex">
                    <div class="textFlow" data-default="select priority">select product</div>
                    <button class="close" type="button"><i class="fa fa-close"></i></button>
                    <button class="drops" type="button"><i class="fa fa-caret-down"></i></button>
                    <input type="hidden" class="selectBox_Value field-check" name="finished_good_id"
                        value="<?= $planning->finished_good_id ?>">
                </div>
                <ul role="listbox" class="selectBox_Container alldiv">
                    <?php
                    foreach ($product as $value) {
                        ?>
                        <li role="option" data-value="<?php echo $value['finished_good_id']; ?>">
    ...
    <div class="form-width-2">
        <div class="form-group field-required ">
            ...
            <input type="text" name="stock" class="form_control field-check" value="<?= $planning->stock ?>" />
            <p class="error-text"></p>
        </div>
    </div>
    ...
    <div class="form-width-2">
        <div class="form-group field-required ">
            ...
            <input type="date" class="form_control field-check" id="finished_date" name="finished_date" value="<?= $planning->finished_date; ?>" />
            <p class="error-text"></p>
        </div>
    </div>
    ...
    <div class="form-width-2">
        <div class="form-group field-required">
            ...
            <div class="selectBox poR">
                <div class="selectBoxBtn flex">
                    <div class="textFlow" data-default="<?= $planning->price_name." [".$planning->price_amount."]" ?>"><?= $planning->price_name." [".$planning->price_amount."]" ?></div>
                    <button class="close" type="button"><i class="fa fa-close"></i></button>
                    <button class="drops" type="button"><i class="fa fa-caret-down"></i></button>
                    <input type="hidden" class="selectBox_Value field-check" name="price_id"
                        value="<?= $planning->price_id ?>">
                </div>
                <ul role="listbox" class="selectBox_Container alldiv">
                <?php
                    if (!empty($price_list)) {
                        foreach ($price_list as $val) {
                            ?>
                            <li role="option" data-value="<?php echo $val['price_id']; ?>">
                                <?php echo $val['name'] . " [" . $val['amount'] . "]"; ?></li>
                            <?php
                        }
                    }
                    ?>
    ...
    <div class="form-width-1">
        <div class="form-group textRight">
            ...
            <button class="btn bg-primary" type="button" id="planning_edit_submit">Save</button>
        </div>
    </div>
</div>
</form>
...
<script type="text/javascript">
    ...
    let form = document.getElementById("planning_edit_form");
    let validator = new FormValidate(form);

    let lock = false;
    document.getElementById("planning_edit_submit").onclick = function (evt) {
        if (!lock) {
            lock = true;
            validator.validate(
                (params) => {
