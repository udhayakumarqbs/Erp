...
class CustomFieldModel extends Model
{
    ...
    protected $validationMessages = [
        ...
        'field_related_to' => [
            'required' => 'The field related to is required.',
        ],
        'field_name' => [
    ...
    public function get_custom_fields_for_form($related)
    {
        $html = '';
        $fields = $this->db->table('custom_fields')
            ->select('*')
            ->where('can_be_purged', 0)
            ->where('field_related_to', $related)
        ...
        foreach ($fields as $field) {
            switch ($field['field_type']) {
                ...
                case "date":
                case "number":
                    $html .= $this->html_input_field($field, $field['field_type']);
                    break;
                case "checkbox":
    ...
    public function get_custom_fields_for_edit($related, $related_id)
    {
        ...
        $builder->select('custom_fields.cf_id, IFNULL(custom_field_values.field_value, "") AS field_value, custom_fields.field_name, custom_fields.field_type, custom_fields.required, custom_fields.field_options');
        $builder->join('custom_field_values', 'custom_field_values.cf_id=custom_fields.cf_id', 'LEFT');
        $builder->where('can_be_purged', 0);
        $builder->where('field_related_to', $related);
        $builder->where("( ISNULL(related_id) OR related_id = {$related_id} )");
    ...
    {
        $html = '';
        $fields = $this->db->query("SELECT field_name, GROUP_CONCAT(field_value SEPARATOR ',') AS field_value FROM custom_field_values JOIN custom_fields ON custom_field_values.cf_id=custom_fields.cf_id WHERE can_be_purged=0 AND field_related_to='$related' AND related_id=$related_id GROUP BY custom_field_values.cf_id ORDER BY order_num ASC ")->getResultArray();
        foreach ($fields as $field) {
            $html .= '
                <tr>
